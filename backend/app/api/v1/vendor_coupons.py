...
@router.get("")
async def list_coupons(is_active: bool = None, page: int = Query(1, ge=1), size: int = Query(20, ge=1, le=100), vid: UUID = Depends(_vendor_id), db: AsyncSession = Depends(get_db)):
    svc = CouponService(db)
    items, total = await svc.list_coupons(vid, is_active, page, size)
