...
async def ensure_crm_tables() -> None:
    ...
    stmts = [
        ...
        # Lead conversion tracking (sfq002)
        "ALTER TABLE crm_chat_conversation ADD COLUMN IF NOT EXISTS converted_lead_id UUID REFERENCES crm_lead(id) ON DELETE SET NULL",
        "ALTER TABLE crm_chat_conversation ADD COLUMN IF NOT EXISTS converted_at TIMESTAMPTZ",
        "CREATE INDEX IF NOT EXISTS ix_crm_chat_conv_lead ON crm_chat_conversation(converted_lead_id)",
        # Ticket conversion tracking (sfq003)
        "ALTER TABLE crm_chat_conversation ADD COLUMN IF NOT EXISTS converted_ticket_id UUID REFERENCES crm_ticket(id) ON DELETE SET NULL",
        "ALTER TABLE crm_chat_conversation ADD COLUMN IF NOT EXISTS ticket_converted_at TIMESTAMPTZ",
        "CREATE INDEX IF NOT EXISTS ix_crm_chat_conv_ticket ON crm_chat_conversation(converted_ticket_id)",
        # Phone capture (sfq004)
        "ALTER TABLE crm_chat_conversation ADD COLUMN IF NOT EXISTS visitor_phone VARCHAR(40)",
        # Chat messages
...
async def ensure_storefront_contact_query_table() -> None:
    ...
    stmts = [
        ...
        # Lead conversion + reply tracking columns (added sfq002)
        "ALTER TABLE storefront_contact_query ADD COLUMN IF NOT EXISTS converted_lead_id UUID REFERENCES crm_lead(id) ON DELETE SET NULL",
        "ALTER TABLE storefront_contact_query ADD COLUMN IF NOT EXISTS converted_at TIMESTAMPTZ",
        "ALTER TABLE storefront_contact_query ADD COLUMN IF NOT EXISTS last_reply_at TIMESTAMPTZ",
        "ALTER TABLE storefront_contact_query ADD COLUMN IF NOT EXISTS reply_count INTEGER NOT NULL DEFAULT 0",
        "CREATE INDEX IF NOT EXISTS ix_scq_converted_lead ON storefront_contact_query(converted_lead_id)",
        # Ticket conversion columns (added sfq003)
        "ALTER TABLE storefront_contact_query ADD COLUMN IF NOT EXISTS converted_ticket_id UUID REFERENCES crm_ticket(id) ON DELETE SET NULL",
        "ALTER TABLE storefront_contact_query ADD COLUMN IF NOT EXISTS ticket_converted_at TIMESTAMPTZ",
...
async def ensure_product_soft_delete() -> None:
    """Add product.deleted_at for trash / restore."""
    if "postgresql" not in settings.DATABASE_URL.lower():
        return
    async with engine.begin() as conn:
        await conn.execute(
            text("ALTER TABLE product ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ")
        )
        await conn.execute(
            text("CREATE INDEX IF NOT EXISTS ix_product_deleted_at ON product (deleted_at)")
        )
    logger.info("ensure_product_soft_delete: product.deleted_at ready")
...
async def ensure_pharma_schema() -> None:
    """Pharmaceutical manufacturing module tables + product/SLoc foundation columns."""
    if "postgresql" not in settings.DATABASE_URL.lower():
        return

    stmts = [
        # Phase 0 — product batch flags
        ...
        # Phase 0 — storage location stock type
        ...
        "ALTER TABLE pharma_batch_sequence ADD COLUMN IF NOT EXISTS period_key VARCHAR(10) NOT NULL DEFAULT ''",
        ...
        # Stage C — product / SLoc / batch / customer GDP + T&T columns
        ...
        # Phase 10 — trading partners then EPCIS events
        ...
        "CREATE INDEX IF NOT EXISTS ix_pharma_wlic_hist_vendor ON pharma_wholesale_license_history (vendor_id)",
        "CREATE INDEX IF NOT EXISTS ix_pharma_wlic_hist_customer ON pharma_wholesale_license_history (vendor_id, customer_id)",
        ...
        "CREATE INDEX IF NOT EXISTS ix_pharma_wlic_doc_vendor ON pharma_wholesale_license_document (vendor_id)",
        "CREATE INDEX IF NOT EXISTS ix_pharma_wlic_doc_customer ON pharma_wholesale_license_document (vendor_id, customer_id)",
    ...
    async with engine.begin() as conn:
        for stmt in stmts:
            await conn.execute(text(stmt))
        # Unique scope for sequences (nullable plant/product) — best-effort index
        await conn.execute(text(
            ...
            "COALESCE(product_id, '00000000-0000-0000-0000-000000000000'::uuid), prefix, period_key)"
        ))

        # Stage B: OOS columns on pharma_inspection_lot
        for col_sql in [
            "ALTER TABLE pharma_inspection_lot ADD COLUMN IF NOT EXISTS oos_status VARCHAR(20)",
            "ALTER TABLE pharma_inspection_lot ADD COLUMN IF NOT EXISTS oos_data JSONB",
        ]:
            await conn.execute(text(col_sql))

        # Stage B: pharma_complaint table
        await conn.execute(text("""
            CREATE TABLE IF NOT EXISTS pharma_complaint (
                id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
                vendor_id UUID NOT NULL REFERENCES vendor(id) ON DELETE CASCADE,
                number VARCHAR(60) NOT NULL,
                complaint_type VARCHAR(30) NOT NULL DEFAULT 'customer',
                severity VARCHAR(20) NOT NULL DEFAULT 'minor',
                title VARCHAR(255) NOT NULL,
                description TEXT,
                goods_batch_id UUID REFERENCES goods_batch(id) ON DELETE SET NULL,
                ...
                created_at TIMESTAMPTZ DEFAULT now(),
                updated_at TIMESTAMPTZ DEFAULT now(),
                CONSTRAINT uq_pharma_complaint_number UNIQUE (vendor_id, number)
            )
        """))
        await conn.execute(text(
            "CREATE INDEX IF NOT EXISTS ix_pharma_complaint_vendor ON pharma_complaint (vendor_id)"
        ))
        await conn.execute(text(
            "CREATE INDEX IF NOT EXISTS ix_pharma_complaint_status ON pharma_complaint (vendor_id, status)"
        ))

    logger.info("ensure_pharma_schema: pharma manufacturing tables ready")
