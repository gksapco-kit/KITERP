...
import { createDefaultTabCategory } from '../../lib/tabsDefaults'
...
interface TabsPropertiesFieldsProps {
  block: Block
  onChange: (props: Partial<Block['props']>) => void
}

export function TabsPropertiesFields({ block, onChange }: TabsPropertiesFieldsProps) {
  const p = block.props
  const categories = p.tabCategories ?? []
  const [expandedCat, setExpandedCat] = useState<number | null>(categories.length > 0 ? 0 : null)
  const [expandedItem, setExpandedItem] = useState<string | null>(null)

  const updateCategories = (next: TabCategory[]) => onChange({ tabCategories: next })
  ...
  const addCategory = () => {
    const next = [...categories, createDefaultTabCategory(`Category ${categories.length + 1}`)]
    updateCategories(next)
  ...
  return (
    <div className="space-y-4 rounded-lg border border-gray-200 bg-white p-3">
      <p className="text-xs font-semibold uppercase tracking-wider text-gray-400">Category tabs</p>
      <p className="text-xs text-gray-500">
        Each category becomes a tab. Items under that category show as cards when the tab is selected.
      </p>

      <Field label="Section title">
        <input className={inputClass} value={p.text ?? ''} onChange={(e) => onChange({ text: e.target.value })} placeholder="Browse by category" />
      </Field>
      ...
      <ColumnsInput value={p.columns ?? 2} onChange={(columns) => onChange({ columns })} min={1} max={4} />
      ...
      <div>
        ...
        {categories.length === 0 && (
          <p className="rounded-lg border border-dashed border-gray-200 py-6 text-center text-xs text-gray-400">
            No categories yet. Click <strong>Add category</strong> to create your first tab.
          </p>
        )}
              ...
              <div key={cat.id} className="rounded-lg border border-gray-200 bg-gray-50">
                ...
                {catOpen && (
                  <div className="space-y-3 border-t border-gray-200 bg-white p-3">
                    ...
                    <div>
                      <div className="mb-2 flex items-center justify-between">
                        <span className="text-xs font-medium text-gray-600">Items in this category</span>
                        <button
                          type="button"
                          onClick={() => addItem(ci)}
                      ...
                      {cat.items.length === 0 && (
                        <p className="text-xs text-gray-400">No cards yet — add items for this tab.</p>
                      )}
                                ...
                                <div className="space-y-2 border-t border-gray-100 bg-white p-2">
                                  <Field label="Title">
                                    <input
                                      className={inputClass}
                                      value={item.title}
                                      onChange={(e) => updateItem(ci, ii, { ...item, title: e.target.value })}
                                    />
                                  </Field>
                                  <Field label="Description">
                                    <textarea
                                      ...
                                      onChange={(e) => updateItem(ci, ii, { ...item, description: e.target.value })}
                                    />
                                  </Field>
                                  <ImageUploadField
                                    label="Image"
                                    value={item.imageUrl}
                                    onChange={(url) => updateItem(ci, ii, { ...item, imageUrl: url })}
                                  />
                                  <Field label="Badge">
                                    <input
                                      className={inputClass}
                                      value={item.badge ?? ''}
                                      onChange={(e) => updateItem(ci, ii, { ...item, badge: e.target.value })}
                                    />
                                  </Field>
                                  <Field label="Price">
                                    <input
                                      className={inputClass}
                                      value={item.price ?? ''}
                                      onChange={(e) => updateItem(ci, ii, { ...item, price: e.target.value })}
                                    />
                                  </Field>
                                  <Field label="Button text">
                                    <input
                                      ...
                                      onChange={(e) => updateItem(ci, ii, { ...item, buttonText: e.target.value })}
                                    />
                                  </Field>
                                  <Field label="Button link">
                                    <input
                                      className={inputClass}
                                      value={item.link ?? ''}
                                      onChange={(e) => updateItem(ci, ii, { ...item, link: e.target.value })}
                                      placeholder="#products"
                                    />
                                  </Field>
                                  <ItemContentStyleFields
                                    style={item.contentStyle}
                                    onChange={(contentStyle) => updateItem(ci, ii, { ...item, contentStyle })}
