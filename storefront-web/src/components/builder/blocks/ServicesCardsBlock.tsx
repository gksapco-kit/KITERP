      ...
      {items.length === 0 ? (
        ...
        <div className={`grid ${colClass[columns] || colClass[3]} gap-6`}>
          {items.map(item => (
            <div
              key={item.id}
              className="builder-tile-card bg-white rounded-2xl border border-gray-100 p-6 hover:shadow-lg hover:-translate-y-1 transition-all duration-200 flex flex-col overflow-hidden"
            >
