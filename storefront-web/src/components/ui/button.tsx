  ...
  'inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium transition-colors disabled:pointer-events-none disabled:opacity-50',
  {
    variants: {
      variant: {
        ...
        secondary: 'bg-secondary text-secondary-foreground hover:bg-secondary/80 ' + focusRingClassName,
        destructive: 'bg-red-500 text-white hover:bg-red-600 ' + solidButtonFocusClassName,
        outline:
        ...
        cancel:
          ...
          'active:bg-[#ffc954]/85 ' +
          focusRingClassName,
        ghost: 'text-foreground hover:bg-muted hover:text-foreground ' + focusRingClassName,
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(({ className, variant, size, asChild = false, ...props }, ref) => {
  ...
  return <Comp className={cn(
    ...
    (hasSolidPrimaryBgClass(className) || isSolidPrimaryButtonClassName(className)) && variant !== 'default' && variant !== 'destructive' && solidButtonFocusClassName,
    className,
  )} ref={ref} {...props} />
})
