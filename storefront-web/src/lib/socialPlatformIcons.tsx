...
/** Instagram drawn as independent rings so the camera frame never loses its sides. */
const INSTAGRAM_PATHS = [
  // Outer rounded square frame (evenodd hole punches the interior)
  ...
  // Corner flash dot
  'M18.406 4.155a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881z',
] as const
  ...
  if (key === 'instagram') {
    return (
      <svg viewBox="0 0 24 24" aria-hidden className={className} fill="currentColor" style={color ? { color } : undefined}>
        {INSTAGRAM_PATHS.map((d) => (
          <path key={d.slice(0, 24)} d={d} fillRule="evenodd" clipRule="evenodd" />
        ))}
      </svg>
    )
  }
