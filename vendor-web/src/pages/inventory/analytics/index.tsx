...
import {
  TrendingUp, BarChart3, Activity, Heart, AlertCircle, Flame,
  ClipboardCheck, Scale, Truck, Package, FileText, Calendar, RefreshCw,
  Store as StoreIcon,
} from 'lucide-react'
...
import { exportPDF, today } from './exportUtils'
import { OverviewTab } from './tabs/OverviewTab'
import { TurnoverTab } from './tabs/TurnoverTab'
import { MovementTrendTab } from './tabs/MovementTrendTab'
import { StockHealthTab } from './tabs/StockHealthTab'
import { ExpiryRiskTab } from './tabs/ExpiryRiskTab'
import { ShrinkageTab } from './tabs/ShrinkageTab'
import { CountAccuracyTab } from './tabs/CountAccuracyTab'
import { ValuationComparisonTab } from './tabs/ValuationComparisonTab'
import { InTransitTab } from './tabs/InTransitTab'
import { ATPTab } from './tabs/ATPTab'

// ── Tab config ───────────────────────────────────────────────────────────────

type TabKey =
  | 'overview' | 'turnover' | 'trend' | 'health'
  | 'expiry' | 'shrinkage' | 'accuracy' | 'valuation'
  | 'transit' | 'atp'

const TABS: { key: TabKey; label: string; icon: React.ElementType; group: string }[] = [
  { key: 'overview',   label: 'Overview',          icon: BarChart3,       group: '' },
  { key: 'turnover',   label: 'Turnover & DSI',    icon: TrendingUp,      group: 'Movement' },
  { key: 'trend',      label: 'Movement Trend',    icon: Activity,        group: 'Movement' },
  { key: 'health',     label: 'Stock Health',      icon: Heart,           group: 'Position' },
  { key: 'expiry',     label: 'Expiry Risk',       icon: AlertCircle,     group: 'Position' },
  { key: 'shrinkage',  label: 'Shrinkage',         icon: Flame,           group: 'Control' },
  { key: 'accuracy',   label: 'Count Accuracy',    icon: ClipboardCheck,  group: 'Control' },
  { key: 'valuation',  label: 'Valuation Compare', icon: Scale,           group: 'Finance' },
  { key: 'transit',    label: 'In-Transit',        icon: Truck,           group: 'Finance' },
  { key: 'atp',        label: 'Available-to-Promise', icon: Package,      group: 'Finance' },
]
...
export default function InventoryAnalyticsPage() {
  ...
  // Ensure default 30-day window is written to URL filters
  useEffect(() => {
  ...
  async function handlePDFExport() {
    setExporting(true)
    try {
      await exportPDF('analytics-content', `inventory-analytics-${tab}-${today()}.pdf`)
    } finally {
      setExporting(false)
    }
  }
  ...
  // Compute the matching prior period for the "vs previous period" label
  ...
  return (
    <div className="p-6 max-w-[1500px] mx-auto space-y-5">
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Summary & detailed inventory reporting across stock value, movement, health and valuation.
          </p>
        </div>
      ...
      <div className="sticky top-0 z-20 rounded-xl border border-border bg-card/95 backdrop-blur p-3 shadow-sm space-y-3">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Select
            ...
            onChange={v => setFilters({ store_id: v || undefined })}
        ...
        <div className="flex items-center justify-between gap-2 text-xs text-gray-500 px-1">
          <span>
            ...
            {filters.store_id
              ? ` · ${stores.find(s => s.id === filters.store_id)?.name ?? ''}`
              : ' · all units'}
          </span>
      ...
      <div id="analytics-content">
        ...
        {tab === 'turnover'   && <TurnoverTab filters={filters} setFilters={setFilters} refreshKey={refreshKey} />}
        {tab === 'trend'      && <MovementTrendTab filters={filters} setFilters={setFilters} refreshKey={refreshKey} />}
        {tab === 'health'     && <StockHealthTab filters={filters} refreshKey={refreshKey} />}
        {tab === 'expiry'     && <ExpiryRiskTab filters={filters} refreshKey={refreshKey} />}
        {tab === 'shrinkage'  && <ShrinkageTab filters={filters} setFilters={setFilters} refreshKey={refreshKey} />}
        {tab === 'accuracy'   && <CountAccuracyTab filters={filters} refreshKey={refreshKey} />}
        {tab === 'valuation'  && <ValuationComparisonTab filters={filters} refreshKey={refreshKey} />}
        {tab === 'transit'    && <InTransitTab filters={filters} refreshKey={refreshKey} />}
        {tab === 'atp'        && <ATPTab filters={filters} refreshKey={refreshKey} />}
      </div>
    </div>
  )
}
