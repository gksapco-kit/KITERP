      ...
      <div className="flex items-center justify-between mb-4">
        ...
        {editing ? (
          <div className="flex gap-2">
            ...
            <button onClick={() => { onSave(form); setEditing(false) }} className="px-3 py-1.5 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700">Save</button>
          </div>
        ) : (
  ...
  return (
    <div className="mt-6 pt-4 border-t">
      <div className="flex items-center justify-between mb-3">
        ...
        {dirty && (
          <button onClick={saveMembers} className="px-3 py-1.5 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700">
            Save Members
          </button>
        )}
      </div>
      ...
      {showTypeManager && (
        <div className="mb-4 p-4 bg-yellow-50 border border-yellow-200 rounded-xl">
          ...
          <div className="flex gap-2">
            ...
            />
            <button onClick={addCustomType} className="px-3 py-1.5 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700">
              + Add Type
            </button>
          </div>
        </div>
      ...
      {showAdd && (
        <form onSubmit={addDoc} className="mb-5 p-4 bg-gray-50 rounded-xl border space-y-3">
          ...
          <div className="flex gap-2 justify-end">
            ...
            <button type="submit" disabled={uploading} className="flex items-center gap-1.5 px-3 py-1.5 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50">
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={() => setShowForm(!showForm)} className="flex items-center gap-1 px-3 py-1.5 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-gray-50 rounded-xl border p-4 space-y-3">
          ...
          <div className="flex justify-end gap-2">
            ...
            <button type="submit" disabled={submit.isPending} className="px-3 py-1.5 text-sm bg-blue-600 text-white rounded-lg disabled:opacity-50">Submit</button>
          </div>
        </form>
      )}
    </div>
      ...
      {showForm && (
        <div className="bg-gray-50 rounded-xl border p-4 space-y-4">
          ...
          <div className="flex justify-end gap-2">
            ...
            <button onClick={handleSave} disabled={createStructure.isPending} className="px-3 py-1.5 text-sm bg-blue-600 text-white rounded-lg disabled:opacity-50">Save Structure</button>
          </div>
        </div>
      )}
    </div>
    ...
    <div className={`border rounded-xl overflow-hidden transition-all ${task.done ? 'opacity-65' : 'bg-white shadow-sm'}`}>
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-gray-50 border-b">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="font-mono text-[11px] font-bold text-blue-700 tracking-wider shrink-0">{taskNum}</span>
          <button type="button" onClick={copyNum}
            className={`flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium border transition-all shrink-0 ${copied ? 'border-green-300 bg-green-50 text-green-600' : 'border-gray-200 bg-white text-gray-400 hover:text-blue-600 hover:border-blue-300'}`}>
            {copied ? <Check className="w-3 h-3" /> : <Copy className="w-3 h-3" />}
            {copied ? 'Copied!' : 'Copy'}
          </button>
          <span className="text-gray-300 shrink-0">·</span>
          <span className={`inline-flex items-center gap-1 text-[11px] px-2 py-0.5 rounded-full font-medium shrink-0 ${pc.color}`}>
          ...
          editing ? (
            <div className="space-y-3">
              ...
              <div className="flex justify-end gap-2">
                ...
                <button type="button" onClick={saveEdit} className="px-4 py-1.5 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700">Save Changes</button>
              </div>
            </div>
          ) : (
            <div className="space-y-2">
              <div className="flex flex-wrap gap-x-6 gap-y-1.5 text-sm">
                {task.assignedTo && (
                  <span className="flex items-center gap-1.5 text-gray-600">
                    <div className="w-5 h-5 rounded-full bg-blue-100 flex items-center justify-center text-[10px] font-bold text-blue-700">{task.assignedTo[0]?.toUpperCase()}</div>
                    {task.assignedTo}
                  </span>
                )}
                {task.dueDate && (
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {task.comments.map(c => (
                <div key={c.id} className="flex gap-2.5">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 mt-0.5 ${c.type === 'change_reason' ? 'bg-orange-100 text-orange-700' : 'bg-blue-100 text-blue-700'}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      {c.type === 'change_reason' && (
                        <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-orange-100 text-orange-600 font-medium">Change Reason</span>
                      )}
      ...
      {activeTab !== 'comments' && (
        <div className="border-t px-4 py-2 flex gap-2 items-center bg-gray-50/60">
          ...
          <button type="button" onClick={addComment} disabled={!newComment.trim()}
            className="px-3 py-1.5 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-40 transition-colors whitespace-nowrap">
            Post
          </button>
        </div>
      )}
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        {editing ? (
          <div className="flex gap-2">
            <button onClick={() => setEditing(false)} className="px-3 py-1.5 text-sm border rounded-lg hover:bg-gray-50">Cancel</button>
            <button onClick={handleSave} className="px-3 py-1.5 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700">Save</button>
          </div>
        ) : (
          <button onClick={() => setEditing(true)} className="px-3 py-1.5 text-sm border rounded-lg hover:bg-gray-50">Edit</button>
        )}
      ...
      <div className="mt-8">
        ...
        {showTaskForm && (
          <div className="border rounded-xl p-4 mb-4 bg-blue-50/30 space-y-3">
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">New Task</p>
            <div>
