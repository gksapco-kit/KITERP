/**
 * Shared Recharts wrappers used by Sales Manager and Rental Reporting Manager.
 * Moved here from pages/sales/salesCharts.tsx — that file re-exports everything
 * so existing imports are not broken.
 */
import {
...
export type TrendDatum = { date: string; revenue: number; orders: number; units?: number }
        ...
        {!compact && (
          <YAxis
            ...
            tickFormatter={(v: number) =>
              metric === 'revenue' ? compactCurrency(v) : String(v)
            }
          />
        )}
        <Tooltip
          ...
          formatter={(v: number) => [
            metric === 'revenue' ? formatCurrency(v) : v,
...
/**
 * Dual-line trend chart: revenue (left axis) + a count series (right axis).
 * Pass `countLabel` to rename "Orders" to "Bookings" or any other noun.
 */
...
export type CatDatum = { label: string; value: number }
      ...
      >
        ...
        {!compact && (
          <YAxis
            ...
            axisLine={false}
            width={48}
            tickFormatter={(v: number) => (money ? compactCurrency(v) : String(v))}
          />
        )}
        <Tooltip
          {...tooltipStyle}
...
/** Horizontal ranking bars — good for top items. */
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      ...
      >
        ...
        <Tooltip
          {...tooltipStyle}
          formatter={(v: number) => [money ? formatCurrency(v) : v, money ? 'Revenue' : 'Value']}
        />
