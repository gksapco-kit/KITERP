...
>(({ className, children, hideCloseButton = false, onPointerDownOutside, onInteractOutside, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        'fixed left-1/2 top-1/2 z-[110] flex w-full max-h-[min(90dvh,calc(100vh-1.5rem))] min-h-0 -translate-x-1/2 -translate-y-1/2 flex-col overflow-y-auto overscroll-contain rounded-lg border border-border bg-card p-6 shadow-xl duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95',
        className,
      )}
...
function DialogHeader({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div className={cn('sticky top-0 z-20 flex flex-col space-y-1.5 bg-card text-center sm:text-left', className)} {...props} />
  )
}
...
function DialogFooter({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      className={cn(
        'sticky bottom-0 z-20 mt-auto flex flex-col-reverse bg-card pt-4 sm:flex-row sm:justify-end sm:space-x-2',
        className,
      )}
      {...props}
    />
