...
import type { StorefrontTemplateMode } from '@/lib/liveStorefrontUrl'
...
type Props = {
  ...
  onConfirm: (mode: StorefrontTemplateMode) => void
  pending?: boolean
}

export function StorefrontTemplateModeToggle({ mode, onConfirm, pending }: Props) {
  const [pendingMode, setPendingMode] = useState<StorefrontTemplateMode | null>(null)
  ...
  const openConfirm = (next: StorefrontTemplateMode) => {
    ...
    <>
      <div className="flex shrink-0 flex-col items-start gap-1 lg:items-end">
        ...
        <div
          ...
          role="group"
          aria-label="Storefront template assignment mode"
        >
          {OPTIONS.map(opt => {
      ...
      {pendingMode && pendingOption ? (
        ...
        >
          <div
            ...
            role="dialog"
            aria-labelledby="storefront-template-mode-title"
            aria-modal="true"
            onClick={e => e.stopPropagation()}
          >
            <div className="flex items-start justify-between gap-3 border-b border-border px-5 py-4">
              <div className="min-w-0">
                <h2 id="storefront-template-mode-title" className="text-base font-semibold text-foreground">
                  Change storefront template mode?
                </h2>
                <p className="mt-1 text-xs text-muted-foreground">
                  This controls whether all stores share one template or each can have its own.
                </p>
              </div>
              <button
                type="button"
