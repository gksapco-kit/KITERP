/**
 * Unified Purchase Order create/edit form.
 *
 * - omit editingPO  → create a new draft PO
 * - provide editingPO → edit an existing draft PO
 *
...
import {
  useCreatePurchaseOrder, useUpdatePurchaseOrder, useSuppliers, useProducts, useServices,
...
export interface PurchaseOrderFormProps {
  /** Null/undefined = create mode. Provide a draft PO to edit it. */
  editingPO?: PurchaseOrder | null
  /** Called after a successful create or update, with the PO id. */
  onSuccess: (poId: string) => void
  /** Called when the user cancels / clicks Back. */
  onCancel: () => void
}
...
export function PurchaseOrderForm({ editingPO, onSuccess, onCancel }: PurchaseOrderFormProps) {
  const navigate = useNavigate()
  const isEditMode = Boolean(editingPO)
  ...
  // ── Prefill from sessionStorage (create mode only) ─────────────────────────
  ...
  // ── Edit mode: pre-fill from editingPO ────────────────────────────────────
  useEffect(() => {
    if (!editingPO) return
    setSupplierId(editingPO.supplier_id || '')
    setExpectedDate(editingPO.expected_delivery_date ? String(editingPO.expected_delivery_date).slice(0, 10) : '')
    setNotes(editingPO.notes || '')
    setCurrency(editingPO.currency || 'INR')
    setPaymentTerms(editingPO.payment_terms || '')
    setLinkedRequisitionId((editingPO as { requisition_id?: string }).requisition_id || '')
    setLinkedPrNumber((editingPO as { pr_number?: string }).pr_number || '')
    const lines = (editingPO.items ?? []).map(item => {
      const isService = Boolean((item as { service_id?: string }).service_id) || item.item_category === 'service'
      return {
        ...emptyItem(),
        item_type: (isService ? 'service' : 'product') as RequisitionType,
        product_id: (isService ? (item as { service_id?: string }).service_id : item.product_id) || '',
        variant_id: item.variant_id || '',
        quantity: String((item as { quantity_ordered?: number }).quantity_ordered ?? item.quantity ?? ''),
    ...
    if (editingPO.items?.[0]) {
      const firstItem = editingPO.items[0]
      setDest(poDestinationFromLine(
        {
          plant_id: firstItem.plant_id || (editingPO as { plant_id?: string }).plant_id,
          storage_location_id: firstItem.storage_location_id,
        },
        (editingPO as { branch_id?: string; store_id?: string }).branch_id
          || (editingPO as { store_id?: string }).store_id
          || selectedStore?.id || '',
      ))
    }
  ...
  // ── Session storage reads (create mode only) ───────────────────────────────
  useEffect(() => {
  ...
  // Backfill UoM + category + unit cost from product/service master when catalog arrives
  useEffect(() => {
      ...
      const next = prev.map(row => {
        ...
        if (!row.unit_of_measure) {
          const fromVariant = row.variant_id
            ...
            : undefined
          const masterUom = normalizeUom(fromVariant || c?.uom || '') || ''
          if (masterUom) { nextRow = { ...nextRow, unit_of_measure: masterUom }; changed = true }
        }
        ...
        if (!row.unit_cost || row.unit_cost === '0') {
          const masterPrice = c?.type === 'service'
            ? resolveServicePurchasePrice(c as unknown as Service) ?? null
            : resolveProductPurchasePrice(c as unknown as Product) ?? null
  ...
  const applyCopiedPo = useCallback((po: PurchaseOrder) => {
    ...
    setDest(poDestinationFromLine(
      { plant_id: (first as { plant_id?: string })?.plant_id || (po as { plant_id?: string }).plant_id, storage_location_id: (first as { storage_location_id?: string })?.storage_location_id },
      selectedStore?.id || '',
    ))
  ...
  const { handleClose, confirmOpen, cancelConfirm, forceClose } = useGuardedClose(onCancel, isDirty, false)
  ...
  const buildItemsPayload = () => items.map(i => {
    ...
    return {
      ...
      plant_id: show('plant') ? poDestinationToPayload(dest).plant_id : undefined,
      storage_location_id: show('storage_location') ? poDestinationToPayload(dest).storage_location_id : undefined,
    }
  })
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      pr_item_ids: items.map(i => i.pr_item_id).filter(Boolean).length
        ? items.map(i => i.pr_item_id).filter(Boolean) as string[]
        : undefined,
      approvers,
    ...
    try {
      if (isEditMode && editingPO) {
        const updated = await updateMut.mutateAsync({ id: editingPO.id, data: payload as Record<string, unknown> })
        onSuccess((updated as PurchaseOrder)?.id || editingPO.id)
      } else {
        const po = await createMut.mutateAsync(payload)
        onSuccess((po as PurchaseOrder).id)
      }
    } catch { /**/ }
  }, [canSubmit, supplierId, items, expectedDate, notes, currency, paymentTerms, dest, createMut, updateMut, linkedRequisitionId, prPrefill?.requisitionId, primaryApproverId, secondaryApproverId, approverMessage, show, activeCostCenters, catalogMap, productDetails, isEditMode, editingPO, onSuccess])

  const showDestination = show('business_unit') || show('plant') || show('storage_location')
  const isPending = isEditMode ? updateMut.isPending : createMut.isPending
      ...
      <div className="sticky top-0 z-20 flex items-center gap-4 border-b border-gray-200 bg-white px-5 py-3 shadow-sm dark:border-gray-700 dark:bg-gray-900">
        ...
        <div className="min-w-0 flex-1 leading-tight">
          <h1 className="truncate text-base font-semibold text-gray-900 dark:text-gray-100">
            {isEditMode
              ? `Edit ${editingPO?.po_number || 'Purchase Order'}`
          ...
          <p className="text-[11px] text-gray-400">
            {isEditMode
              ? 'Update the draft purchase order fields and save'
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          >
            ...
            {isEditMode ? 'Save Changes' : 'Create Draft PO'}
          </Button>
        </div>
      </div>
                    ...
                    <div key={item.uid} className="flex flex-col gap-2 px-3 py-3 sm:px-4">
                      ...
                      <div className="flex min-w-0 flex-1 flex-col gap-2">
                        ...
                        <div className={LINE_ROW_GRID}>
                          ...
                          <LineField label="Quantity" required={req('quantity')}>
                            <Input
                          ...
                          <LineField label="Unit Cost" required={req('net_price')}>
                            <Input
                              type="number"
                              min={0}
                              step="0.01"
                        ...
                        {/* Row 2 — UoM + master codes / service period */}
                        <div className={LINE_ROW_GRID}>
                          {show('unit') ? (
                            <LineField label="UoM">
                              <Select
                        ...
                        {/* Row 3 — department / priority / date / note */}
                        <div className={LINE_ROW_GRID}>
